(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global["tiptap-extension-global-drag-handle"] = {}, global.core, global.state, global.model, global.pmView));
})(this, (function (exports, core, state, model, pmView) { 'use strict';
  ...
  var pmView__namespace = /*#__PURE__*/_interopNamespace(pmView);

  function getPmView() {
      try {
          return pmView__namespace;
      }
      catch (error) {
          return null;
      }
  ...
  function DragHandlePlugin(options) {
      ...
      function handleDragStart(event, view) {
          ...
          if (nodePos.node().type.name === 'doc')
              ...
              const nodeSelection = state.NodeSelection.create(view.state.doc, nodePos.before());
          ...
          if (!differentNodeSelected &&
              diff !== 0 &&
              !(view.state.selection instanceof state.NodeSelection)) {
              const endSelection = state.NodeSelection.create(view.state.doc, to - 1);
              selection = state.TextSelection.create(view.state.doc, draggedNodePos, endSelection.$to.pos);
          }
          else {
              selection = state.NodeSelection.create(view.state.doc, draggedNodePos);
              ...
              if (selection.node.type.isInline ||
                  ...
                  selection = state.NodeSelection.create(view.state.doc, $pos.before());
              }
          }
      ...
      return new state.Plugin({
          ...
          view: (view) => {
              ...
              if (editorElement) {
                  editorElement.addEventListener('mouseout', mouseOutEventHandler);
              }
              return {
                  destroy: () => {
                      if (!handleBySelector) {
                      ...
                      if (editorElement) {
                          editorElement.removeEventListener('mouseout', mouseOutEventHandler);
                      }
                  },
              };
          },
